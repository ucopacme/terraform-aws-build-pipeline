...
module "eventbridge" {
  ...
  pipeline_arn           = aws_codepipeline.this[0].arn
  create_bus             = false
  create_role            = true
    ...
    Eventbridge = [
      {
        ...
        role_arn               = module.eventbridge[0].eventbridge_role_arn
        attach_pipeline_policy = true
...
resource "aws_iam_policy" "cross_account_put_events" {
  ...
  description = "Permissions for ${module.eventbridge[0].eventbridge_role_name} to put events to other accounts"
